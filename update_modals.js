...
files.forEach(f => {
  ...
  // Her dosyanın sonundaki login modalını ve toast container'ı bul
  const startIndex = text.indexOf('<!-- LOGIN MODAL -->');
  if (startIndex !== -1) {
    const endIndex = text.indexOf('<script src="js/app.js"></script>');
    if (endIndex !== -1) {
      text = text.substring(0, startIndex) + newModals + '\n\n' + text.substring(endIndex);
