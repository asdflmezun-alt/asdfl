-- =========================================================================
-- MİGRASYON SORGUSU: PROFİLLER TABLOSUNA ŞİRKET/KURUM SÜTUNU EKLEME
-- =========================================================================
-- Bu sorguyu Supabase Dashboard üzerindeki "SQL Editor" kısmında çalıştırarak
-- canlı veritabanınızı güncelleyebilirsiniz.

-- 1. Profiles tablosuna company (Şirket/Kurum) sütunu ekleme
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS company TEXT;

-- 2. Yeni üye kaydında bilgileri profiles tablosuna aktaran trigger fonksiyonunu güncelleme
CREATE OR REPLACE FUNCTION public.handle_new_user() 
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (
    id, role, name, email, grad_year, job, company, city, grade, branch, teaching_year, class_section,
    phone, share_phone, share_email
  )
  VALUES (
    NEW.id,
    COALESCE((NEW.raw_user_meta_data->>'role')::text, 'Kullanıcı'),
    COALESCE((NEW.raw_user_meta_data->>'name')::text, split_part(NEW.email, '@', 1)),
    NEW.email,
    NULLIF(NEW.raw_user_meta_data->>'gradYear', '')::INTEGER,
    NEW.raw_user_meta_data->>'job',
    NEW.raw_user_meta_data->>'company',
    NEW.raw_user_meta_data->>'city',
    NEW.raw_user_meta_data->>'grade',
    NEW.raw_user_meta_data->>'branch',
    NULLIF(NEW.raw_user_meta_data->>'teachingYear', '')::INTEGER,
    NEW.raw_user_meta_data->>'classSection',
    NEW.raw_user_meta_data->>'phone',
    COALESCE((NEW.raw_user_meta_data->>'sharePhone')::boolean, false),
    COALESCE((NEW.raw_user_meta_data->>'shareEmail')::boolean, false)
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
